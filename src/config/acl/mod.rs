...
impl SubjectMatch {
    pub fn matches(&self, ctx: &SubjectContext) -> bool {
        let sub_value: Option<Value> = ctx.subject.as_ref().map(|s| Value::String(s.clone()));
        let sub: Option<&Value> = sub_value.as_ref();
        ...
        self.username
            ...
            && self.subject.as_ref().is_none_or(|m| m.matches(sub))
